...
hedittext=uicontrol(f,'Style','edit','String',str,'Max',1,'Min',0,'Position',[315,260,70,25],'KeyPressFcn',{@Key_Press_Callback});
htext = uicontrol('Style','text','String','Select Data','Position',[325,90,60,15]);
ha = axes('Units','pixels','Position',[50,60,200,185]);
align([hsurf,hmesh,hcontour,htext,hpopup],'Center','None');
% Initialize the GUI. 
% Change units to normalized so components resize automatically. 
set([f,hsurf,hmesh,hcontour,htext,hpopup,hedittext,ha],'Units','normalized');
...
set(f,'Visible','on');
    function Key_Press_Callback(source,evendata)
        if get(eventdata.character) =='a'
            
       fprintf('Sauarabh Deshmukh\n');
        end
    end
